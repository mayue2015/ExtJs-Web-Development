...
Ext.app.getBaseData = function() {
	Ext.Msg.confirm("请确认", "是否提取电缆基准参数?", function(button, text) {
		if (button == 'yes') {
			Ext.Ajax.request({
				url : 'sysconfig/dxcswhAction/getBaseData_dl.do',
				params : {
					user_dept : deptguid,
					type : 'first'
				},
...
Ext.app.dataJson = function() {
	Ext.Ajax.request({
		url : 'sysconfig/dxcswhAction/getConductorType.do?voltagelevel='+Ext.app.voltagelevel,
		method : 'post',
		success : function(response, options) {
